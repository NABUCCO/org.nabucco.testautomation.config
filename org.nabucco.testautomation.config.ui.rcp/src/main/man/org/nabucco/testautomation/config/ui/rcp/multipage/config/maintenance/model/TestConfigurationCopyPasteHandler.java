...
import org.nabucco.testautomation.config.ui.rcp.multipage.config.maintenance.masterdetails.TestConfigurationMaintenanceMasterDetailTreeNodeCreator;
...
public class TestConfigurationCopyPasteHandler extends AbstractCopyPasteHandler implements CopyPasteHandler {
    ...
    private TestConfigurationMaintenanceMasterDetailTreeNodeCreator treeNodeCreator;

    public TestConfigurationCopyPasteHandler() {
        this.treeNodeCreator = new TestConfigurationMaintenanceMasterDetailTreeNodeCreator();
    }
        ...
        if (DataModelManager.isAllowedChild(targetNode.getDatatype(), copiedDatatype)) {
            ...
            if (targetDatatype instanceof TestConfiguration && copiedDatatype instanceof TestConfigElement) {
                ...
                TestConfigElementContainer testConfigElementContainer = TestConfigurationElementFactory
                        .clone(copiedTestConfigElement);
                clone = testConfigElementContainer.getElement();
                List<TestConfigElementContainer> testConfigElementList = ((TestConfiguration) targetDatatype)
                ...
                if (copiedDatatype instanceof TestConfigElement) {
                    ...
                    TestConfigElementContainer testConfigElementContainer = TestConfigurationElementFactory
                            .clone(copiedTestConfigElement);
                    clone = testConfigElementContainer.getElement();
                    List<TestConfigElementContainer> testConfigElementList = ((TestConfigElement) targetDatatype)
                            .getTestConfigElementList();
                    testConfigElementContainer.setOrderIndex(testConfigElementList.size());
                    testConfigElementList.add(testConfigElementContainer);
