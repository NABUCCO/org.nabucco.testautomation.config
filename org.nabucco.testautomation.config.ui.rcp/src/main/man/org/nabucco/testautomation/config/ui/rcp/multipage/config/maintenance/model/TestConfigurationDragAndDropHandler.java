...
public class TestConfigurationDragAndDropHandler extends AbstractDragAndDropHandler implements
    ...
    public boolean validateDrop(MasterDetailTreeNode data, MasterDetailTreeNode target, int location) {
        ...
        if (location == DropBefore || location == DropAfter) {
            ...
            if (movedDatatype instanceof PropertyList && location == DropAfter && targetDatatype instanceof TestConfigElement) {
                // PropertyList has to be the first element in Composites
                return false;
            }
            if (movedDatatype instanceof PropertyList && location == DropAfter && targetDatatype instanceof TestConfiguration) {
                return false;
            }
            ...
            if (targetParentDatatype instanceof TestConfiguration) {
                if (movedDatatype instanceof TestConfigElement) {
                    if (Arrays.asList(DataModelManager.getPossibleChildrenTypes(targetParentDatatype)).contains(
                            movedDatatype.getClass())) {
                        if (checkSchema(parentNode, data)) {
                            return verifyHierarchy(data, target);
                        }
                    }
                } else if (movedDatatype instanceof PropertyList) {
                    // No before/after inserts for PropertyLists allowed if target parent is a
                    // TestConfiguration
                    return false;
                }
                return false;
            ...
            if (targetParentDatatype instanceof TestConfigElement) {
                ...
                if (movedDatatype instanceof TestConfigElement) {
                    if (targetParentTestConfigElement.getPropertyList() == null || location == DropAfter) {
                        if (Arrays.asList(DataModelManager.getPossibleChildrenTypes(targetParentDatatype)).contains(
                                movedDatatype.getClass())) {
                            if (checkSchema(parentNode, data)) {
                                return verifyHierarchy(data, target);
                        ...
                        if (targetIndex == 0) {
                            // it is not allowed to insert to position 0, when a propertyList
                            // exists.
                            return false;
                        }
                    }
            ...
            if (targetDatatype instanceof TestConfiguration) {
                if (movedDatatype instanceof TestConfigElement) {
                    if (Arrays.asList(DataModelManager.getPossibleChildrenTypes(targetDatatype)).contains(
                            movedDatatype.getClass())) {
    ...
    public boolean postDrop(MasterDetailTreeNode movedNode, MasterDetailTreeNode targetNode, int location) {
            ...
            if (targetParentDatatype instanceof TestConfiguration) {
                ...
                if (movedDatatype instanceof TestConfigElement) {
                    ...
                    if (movedNode.getParent().getDatatype() instanceof TestConfiguration) {
                        testConfigElementContainerToMove = ((TestConfiguration) movedNode.getParent().getDatatype())
                                .getTestConfigElementList().get(indexOfMovedNodeInOldParent);
                        ...
                        testConfigElementContainerToMove = ((TestConfigElement) movedNode.getParent().getDatatype())
                                .getTestConfigElementList().get(indexOfMovedNodeInOldParent);
                    } else {
                        Activator.getDefault().logDebug(
                                "Fatal Error. Parent of moved node is no TestConfiguration ore TestConfigElement!");
                        return false;
                    }
                    ...
                    // Insert
                    targetTestConfiguration.getTestConfigElementList().add(indexToInsertTo,
                            testConfigElementContainerToMove);
                ...
                if (movedDatatype instanceof TestConfigElement) {
                    ...
                    // Insert
                    targetParentTestConfigElement.getTestConfigElementList().add(indexToInsertTo,
                            testConfigElementContainerToMove);
                    // Update order index of inserted container
                ...
                int indexOfMovedNodeInOldParent = movedNode.getParent().getChildren().indexOf(movedNode);
                PropertyContainer propertyContainer = ((PropertyList) movedNode.getParent().getDatatype())
                        .getPropertyList().get(indexOfMovedNodeInOldParent);
                // Find index to insert to
                int indexToInsertTo = targetNode.getParent().getChildren().indexOf(targetNode);
                if (location == 2) {
                    indexToInsertTo++;
        ...
        if (location == 3) {
            ...
            if (targetDatatype instanceof TestConfiguration) {
                ...
                if (movedNode.getParent().getDatatype() instanceof TestConfigElement) {
                    ...
                    try {
                        propertyContainer = PropertyComponentServiceDelegateFactory.getInstance()
                                .getProduceProperty().produceProperty(rq).getPropertyContainer();
                    ...
                    propertyContainer = ((PropertyList) movedNode.getParent().getDatatype()).getPropertyList().get(
                            indexOfMovedNodeInOldParent);
                }
        ...
        // Perform changes in Tree - We have to do this last, because changes in the tree could
        // influence the indices needed for proper datamodel change
    ...
    public void dragFinished(MasterDetailTreeNode movedNode) {
        ...
        Activator.getDefault().logDebug(
                "Fatal Error. Parent node of dragged Element is no TestConfigElement or PropertyList!");
    }
    ...
    public boolean performUiDrop(MasterDetailTreeNode movedNode, MasterDetailTreeNode targetNode, int location) {
        ...
        if (location == 1 || location == 2) {
            ...
            if (movedNode.getDatatype() instanceof PropertyList
                    && (!(targetNode.getParent().getDatatype() instanceof PropertyContainer))) {
    ...
    private boolean checkSchema(MasterDetailTreeNode parent, MasterDetailTreeNode data) {
        if (parent.getDatatype() instanceof TestConfiguration) {
            ...
            if (data.getDatatype() instanceof TestConfigElement) {
                ...
                return (parentElement.getSchemaElement().getPropertyContainer() != null
                        && parentElement.getSchemaElement().getPropertyContainer().getValue() != null && parentElement
                        .getSchemaElement().getPropertyContainer().getValue());
            }
        }
        return false;
    }

    private boolean verifyHierarchy(MasterDetailTreeNode nodeToMove, MasterDetailTreeNode targetNode) {
