...
*/
package org.nabucco.testautomation.config.ui.rcp.multipage.config.maintainance.masterdetails.detail;
...
import org.nabucco.testautomation.config.ui.rcp.multipage.config.maintainance.masterdetails.widgetcreators.attributevalue.AttributeValueWidgetCreator;
import org.nabucco.testautomation.config.ui.rcp.multipage.config.maintainance.masterdetails.widgetcreators.dependency.DependencyTableWidgetCreator;
import org.nabucco.testautomation.config.ui.rcp.multipage.config.maintainance.masterdetails.widgetcreators.script.TestScriptTableWidgetCreator;
...
TestautomationDetailPageViewLayouter {
	...
	protected Control layoutElement(Composite parent,
		...
		if (propertyName.equalsIgnoreCase(PROPERTY_SCRIPT_LIST)) {
			...
			return this.layoutAttribute(parent, widgetFactory, datatype,
					masterBlockId, property, propertyName, data, readOnly,
					externalViewModel, messageManager);
	 ...
	 */
	private Control layoutAttribute(Composite parent,
			BaseTypeWidgetFactory widgetFactory, Datatype datatype,
			String masterBlockId, Object property, String propertyName,
			GridData data, boolean readOnly, ViewModel externalViewModel,
			NabuccoMessageManager messageManager) {
		...
		Control result = null;
		TestConfigElement configElement = (TestConfigElement) datatype;
		List<AttributeValue> attributeValueList = configElement
		.getAttributeValueList();
		for (AttributeValue attributeValue : attributeValueList) {

			Map<String, Serializable> labelMap = new HashMap<String, Serializable>();
			labelMap.put("name", attributeValue.getAttribute().getName()
					.getValue());
			Label label = widgetFactory.createLabel(parent, masterBlockId + "."
					+ propertyName, labelMap);
			label.setToolTipText(label.getText());
			label.setLayoutData(data);

			NabuccoFormToolkit nft = widgetFactory.getNabuccoFormToolKit();

			AttributeValueWidgetCreator widgetCreator = new AttributeValueWidgetCreator(
					nft);
			Control newWidget = widgetCreator.createWidget(parent, configElement,
					attributeValue, null, attributeValue, readOnly,
					externalViewModel, messageManager, propertyName);
			
			if (newWidget == null) {
				Activator.getDefault().logError(
				"Cannot create AttributeValue Widget [null].");
	...
	private Control layoutExecutionType(Composite parent,
		...
		Control newWidget = widgetCreator.createViewInputElement(parent, property, setter, configElement, readOnly, externalViewModel, messageManager, propertyName);
	...
	private Control layoutSciptTable(Composite parent,
		...
		TestScriptTableWidgetCreator widgetCreator = new TestScriptTableWidgetCreator(
				nft);
		Control newWidget = widgetCreator.createViewInputElement(parent,
				property, null, datatype, readOnly, externalViewModel,
				messageManager, propertyName);

		if (newWidget == null) {
			Activator.getDefault().logError(
			"Cannot create TestScriptPicker Widget [null].");
			newWidget = nft.createRealLabel(parent, "INVALID");
		} else {
			super.setDataForWidget(data, newWidget);
		}
	...
	private Control layoutDependencyTable(Composite parent,
		...
		DependencyTableWidgetCreator widgetCreator = new DependencyTableWidgetCreator(
				nft);
		Control newWidget = widgetCreator.createViewInputElement(parent,
				property, null, datatype, readOnly, externalViewModel,
				messageManager, propertyName);
		...
		if (newWidget == null) {
			Activator.getDefault().logError(
			"Cannot create DependencyPicker Widget [null].");
			newWidget = nft.createRealLabel(parent, "INVALID");
		} else {
			super.setDataForWidget(data, newWidget);
		}
